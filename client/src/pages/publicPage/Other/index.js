...
const Modal = () => {
  ...
  return (
    <>
      <Title title={'useRef'}/>
      <button className={'btn btn-big'} onClick={handleChange}>Open modal</button>
      {modal
...
const Memo = () => {
  ...
  return (
    <>
      <Title title={'memo'}/>
      <p>Count: {count}</p>
      <button className={'btn btn-big'} onClick={()=>setCount(prev => prev+1)}>Click</button>
    </>
  )
}
...
const UseMemo = () => {
  ...
  return (
    <>
      ...
      <button className={'btn btn-big'} onClick={()=>setUpdate(prev => !prev)}>Click</button>
    </>
  )
}
