...
const PostsList = React.lazy(()=> import('components/DashboardPosts/PostsList'));
...
function Posts() {
  ...
  const updatePosts = (e, id, post) => {
    e.preventDefault();
    const updatePosts = posts.map(item => item.id === id ? {...item, ...post} : item);
  ...
  return (
    <>
      <div className={'dashboard-header'}>
        ...
        <button className={'btn-icon'} onClick={openModal}><EditNote /></button>
      </div>
      {posts.length
      ...
      {modal
        ? (<Modal closeModal={closeModal}>
            <PostsForm addedPosts={addedPosts} closeModal={closeModal} />
          </Modal>)
        : null}
    </>
  );
